...
void PixelsTable::ClearColor(const Vector4D<Uint8>& drawing_color) {
  for (int y = 0; y < window_dimensions_.y_; y++) {
    for (int x = 0; x < window_dimensions_.x_; x++) {
      pixels_[kNumberOfColors * (y * window_dimensions_.x_ + x) + kRedPosition] = drawing_color.x_;
      pixels_[kNumberOfColors * (y * window_dimensions_.x_ + x) + kGreenPosition] = drawing_color.y_;
      pixels_[kNumberOfColors * (y * window_dimensions_.x_ + x) + kBluePosition] = drawing_color.z_;
      pixels_[kNumberOfColors * (y * window_dimensions_.x_ + x) + kAlphaPosition] = drawing_color.w_;
    }
  }
}
